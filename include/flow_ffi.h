...
#define FLOW_FFI_EXPORT __attribute__((visibility("default")))
     ...
     *         port_key.
     */
    FLOW_FFI_EXPORT FlowError flow_env_add_task_set_input_data(FlowEnvHandle env, FlowNodeHandle node,
    ...
    FLOW_FFI_EXPORT FlowNodeHandle flow_graph_add_node(FlowGraphHandle graph, const char* class_id, const char* name);
    ...
    FLOW_FFI_EXPORT FlowNodeHandle flow_graph_add_node_with_uuid(FlowGraphHandle graph, const char* class_id,
    ...
    // Get all nodes
    FLOW_FFI_EXPORT FlowError flow_graph_get_nodes(FlowGraphHandle graph, FlowNodeHandle** nodes, size_t* count);

    // Connect two nodes
    FLOW_FFI_EXPORT FlowConnectionHandle flow_graph_connect_nodes(FlowGraphHandle graph, const char* source_id,
                                                                  const char* source_port, const char* target_id,
                                                                  const char* target_port);

    // Disconnect nodes
    FLOW_FFI_EXPORT FlowError flow_graph_disconnect_nodes(FlowGraphHandle graph, const char* connection_id);

    // Get all connections in the graph
    FLOW_FFI_EXPORT FlowError flow_graph_get_connections(FlowGraphHandle graph, FlowConnectionInfo** connections,
                                                         size_t* count);

    // Check if two nodes can be connected
    FLOW_FFI_EXPORT bool flow_graph_can_connect(FlowGraphHandle graph, const char* source_id, const char* source_port,
                                                const char* target_id, const char* target_port);
    ...
    FLOW_FFI_EXPORT FlowNodeDataHandle flow_node_get_input_data(FlowNodeHandle node, const char* port_key);

    FLOW_FFI_EXPORT FlowNodeDataHandle flow_node_get_output_data(FlowNodeHandle node, const char* port_key);
    ...
    // Port introspection
    FLOW_FFI_EXPORT FlowError flow_node_get_input_port_keys(FlowNodeHandle node, char*** port_keys, size_t* count);

    FLOW_FFI_EXPORT FlowError flow_node_get_output_port_keys(FlowNodeHandle node, char*** port_keys, size_t* count);

    // Get port type information
    FLOW_FFI_EXPORT const char* flow_node_get_input_port_type(FlowNodeHandle node, const char* port_key);

    FLOW_FFI_EXPORT const char* flow_node_get_output_port_type(FlowNodeHandle node, const char* port_key);

    // Get port description/caption
    FLOW_FFI_EXPORT const char* flow_node_get_port_description(FlowNodeHandle node, const char* port_key,
                                                               bool is_input_port);
    ...
    FLOW_FFI_EXPORT FlowError flow_node_get_input_ports_metadata(FlowNodeHandle node, FlowPortMetadata** metadata_array,
                                                                 size_t* count);
    ...
    FLOW_FFI_EXPORT FlowNodeHandle flow_factory_create_node(FlowNodeFactoryHandle factory, const char* class_name,
                                                            const char* uuid, const char* name, FlowEnvHandle env);

    // Get available node categories
    FLOW_FFI_EXPORT FlowError flow_factory_get_categories(FlowNodeFactoryHandle factory, char*** categories,
                                                          size_t* count);

    // Get node classes in a category
    FLOW_FFI_EXPORT FlowError flow_factory_get_node_classes(FlowNodeFactoryHandle factory, const char* category,
                                                            char*** classes, size_t* count);

    // Get friendly name for a node class
    FLOW_FFI_EXPORT const char* flow_factory_get_friendly_name(FlowNodeFactoryHandle factory, const char* class_name);

    // Type conversion checking
    FLOW_FFI_EXPORT bool flow_factory_is_convertible(FlowNodeFactoryHandle factory, const char* from_type,
                                                     const char* to_type);
    ...
    FLOW_FFI_EXPORT FlowNodeDataHandle flow_data_create_image_encoded(const uint8_t* bytes, size_t length,
    ...
    FLOW_FFI_EXPORT FlowNodeDataHandle flow_data_create_image_raw(const uint8_t* bytes, int32_t width, int32_t height,
                                                                  int32_t row_stride_bytes, FlowPixelFormat format);
    ...
    FLOW_FFI_EXPORT FlowError flow_data_image_borrow(FlowNodeDataHandle data, FlowImageDescriptor* out_desc);
    ...
    typedef int (*FlowTextureCreateFn)(void* registrar, int32_t width, int32_t height, void** out_texture_object,
                                       int64_t* out_texture_id, uint32_t* out_gl_name);

    typedef void (*FlowTextureDestroyFn)(void* registrar, void* texture_object, int64_t texture_id);

    typedef void (*FlowTextureMarkFrameFn)(void* registrar, void* texture_object);
    ...
    typedef int (*FlowTextureWaitInitFn)(void* texture_object, void** out_cuda_resource);
    ...
    typedef int (*FlowTextureSubmitFrameFn)(void* texture_object, const void* src_device_ptr, int32_t src_pitch_bytes,
    ...
    //     *out_texture_id     — int64_t Flutter texture ID
    //   Returns 0 on success, -1 on failure.
    typedef int (*FlowGpuTextureCreateFn)(void* registrar, int32_t width, int32_t height, void** out_texture_object,
    ...
    typedef void (*FlowGpuTextureDestroyFn)(void* registrar, void* texture_object, int64_t texture_id);

    // mark_gpu_frame_available(registrar, texture_object)
    //   Call fl_texture_registrar_mark_texture_frame_available.
    typedef void (*FlowGpuTextureMarkFrameFn)(void* registrar, void* texture_object);
    ...
    typedef int (*FlowGpuTextureSubmitFrameFn)(void* texture_object, const void* src_bytes, int32_t row_stride_bytes,
                                               int32_t width, int32_t height);
    ...
    FLOW_FFI_EXPORT FlowError flow_ffi_register_flutter_texture(int32_t w, int32_t h, FlowGpuTextureHandle* out,
                                                                int64_t* out_texture_id);

    // Unregister a previously registered Flutter texture.
    FLOW_FFI_EXPORT FlowError flow_ffi_unregister_flutter_texture(FlowGpuTextureHandle handle);
    ...
    FLOW_FFI_EXPORT FlowError flow_ffi_upload_to_texture(FlowGpuTextureHandle handle, const void* host_bytes,
                                                         int32_t row_stride_bytes, int32_t width, int32_t height);
    ...
    typedef void (*FlowNodeDataEventCallback)(FlowNodeHandle node, const char* port_key, FlowNodeDataHandle data,
                                              void* user_data);
    ...
    FLOW_FFI_EXPORT FlowEventRegistrationHandle flow_graph_on_node_removed(FlowGraphHandle graph,
                                                                           FlowNodeEventCallback callback,
                                                                           void* user_data);

    FLOW_FFI_EXPORT FlowEventRegistrationHandle flow_graph_on_nodes_connected(FlowGraphHandle graph,
                                                                              FlowConnectionEventCallback callback,
                                                                              void* user_data);

    FLOW_FFI_EXPORT FlowEventRegistrationHandle flow_graph_on_nodes_disconnected(FlowGraphHandle graph,
                                                                                 FlowConnectionEventCallback callback,
                                                                                 void* user_data);
    ...
    FLOW_FFI_EXPORT FlowEventRegistrationHandle flow_node_on_error(FlowNodeHandle node, FlowErrorEventCallback callback,
                                                                   void* user_data);

    FLOW_FFI_EXPORT FlowEventRegistrationHandle flow_node_on_set_input(FlowNodeHandle node,
                                                                       FlowNodeDataEventCallback callback,
                                                                       void* user_data);

    FLOW_FFI_EXPORT FlowEventRegistrationHandle flow_node_on_set_output(FlowNodeHandle node,
                                                                        FlowNodeDataEventCallback callback,
                                                                        void* user_data);
