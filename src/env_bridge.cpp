...
extern "C"
{
    ...
    FLOW_FFI_EXPORT FlowNodeFactoryHandle flow_env_get_factory(FlowEnvHandle env)
    {
        FLOW_API_CALL_HANDLE({
            ...
            if (!factory)
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_UNKNOWN, "Environment has no factory");
                return nullptr;
            }

            // Create wrapper and handle
            auto wrapper = NodeFactoryWrapper(factory);
            return reinterpret_cast<FlowNodeFactoryHandle>(flow_ffi::create_handle<NodeFactoryWrapper>(wrapper));
        });
    }
