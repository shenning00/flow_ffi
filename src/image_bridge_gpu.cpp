...
static std::unordered_map<FlowGpuTextureHandle, std::unique_ptr<GpuTextureState>> g_registry;

// Diagnostic counter — see flow_ffi_upload_to_texture.
static std::atomic<uint64_t> g_upload_count{0};
...
extern "C" FlowError flow_ffi_register_flutter_texture(int32_t w, int32_t h, FlowGpuTextureHandle* out,
                                                       int64_t* out_texture_id)
{
    ...
    if (w <= 0 || h <= 0)
    {
        flow_set_error(FLOW_ERROR_INVALID_ARGUMENT, "flow_ffi_register_flutter_texture: "
    ...
    {
        flow_set_error(FLOW_ERROR_NOT_IMPLEMENTED, "flow_ffi_register_flutter_texture: "
    ...
    {
        flow_set_error(FLOW_ERROR_UNKNOWN, "flow_ffi_register_flutter_texture: "
...
extern "C" FlowError flow_ffi_unregister_flutter_texture(FlowGpuTextureHandle handle)
{
    std::fprintf(stderr, "[image_bridge_gpu] unregister_flutter_texture: handle=%p\n", handle);
    std::fflush(stderr);

    if (!handle)
    {
        flow_set_error(FLOW_ERROR_INVALID_HANDLE, "flow_ffi_unregister_flutter_texture: null handle");
    ...
    std::unique_ptr<GpuTextureState> state;
    {
        ...
        {
            flow_set_error(FLOW_ERROR_INVALID_HANDLE, "flow_ffi_unregister_flutter_texture: "
    ...
    if (state->flutter_texture_object)
    {
        if (flow_ffi_is_gpu_texture_ops_bound())
        {
            FlowGpuTextureOps ops = flow_ffi_get_gpu_texture_ops();
            void* registrar       = flow_ffi_get_texture_registrar();
            ops.destroy_texture(registrar, state->flutter_texture_object, state->flutter_texture_id);
        }
...
extern "C" FlowError flow_ffi_upload_to_texture(FlowGpuTextureHandle handle, const void* host_bytes,
                                                int32_t row_stride_bytes, int32_t width, int32_t height)
{
    // Diagnostic: log first call + every Nth so we know the upload path is alive.
    const uint64_t n = g_upload_count.fetch_add(1, std::memory_order_relaxed) + 1;
    if (n == 1 || (n % 100) == 0)
    {
        std::fprintf(stderr, "[image_bridge_gpu] upload_to_texture#%lu handle=%p %dx%d stride=%d\n",
                     static_cast<unsigned long>(n), handle, width, height, row_stride_bytes);
        std::fflush(stderr);
    }
    ...
    void* tex_obj = nullptr;
    {
        ...
        if (!s)
        {
            flow_set_error(FLOW_ERROR_INVALID_HANDLE, "flow_ffi_upload_to_texture: handle not in registry");
            return FLOW_ERROR_INVALID_HANDLE;
        }
        if (width != s->width || height != s->height)
        {
            flow_set_error(FLOW_ERROR_INVALID_ARGUMENT, "flow_ffi_upload_to_texture: dimensions mismatch — "
    ...
    int submit_rc = ops.submit_frame(tex_obj, host_bytes, row_stride_bytes, width, height);
