...
extern "C"
{
                                                            ...
                                                            const char* uuid, const char* name, FlowEnvHandle env)
    {
        FLOW_API_CALL_HANDLE({
            ...
            if (!factory_wrapper)
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_INVALID_HANDLE, "Invalid factory handle");
                return nullptr;
            }
            ...
            if (!env_wrapper)
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_INVALID_HANDLE, "Invalid environment handle");
                return nullptr;
            }
            ...
            try
            {
                ...
                auto node = factory_wrapper->factory->CreateNode(class_name, node_uuid, node_name, env_wrapper->env);

                if (!node)
                {
                    flow_ffi::ErrorManager::instance().set_error(
                        FLOW_ERROR_NODE_NOT_FOUND, std::string("Failed to create node of class: ") + class_name);
                    return nullptr;
                }

                return reinterpret_cast<FlowNodeHandle>(flow_ffi::get_or_create_node_handle(node));
            }
            catch (const std::invalid_argument& e)
            {
                flow_ffi::ErrorManager::instance().set_error(
                    FLOW_ERROR_INVALID_ARGUMENT, std::string("Invalid UUID string or argument: ") + e.what());
                return nullptr;
            }
            catch (const std::exception& e)
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_UNKNOWN,
                                                             std::string("Node creation failed: ") + e.what());
                return nullptr;
            }
        });
    }
                                                          ...
                                                          size_t* count)
    {
        FLOW_API_CALL({
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_UNKNOWN,
                                                             std::string("Failed to get categories: ") + e.what());
                                                            ...
                                                            char*** classes, size_t* count)
    {
        FLOW_API_CALL({
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_UNKNOWN,
                                                             std::string("Failed to get node classes: ") + e.what());
                return FLOW_ERROR_UNKNOWN;
            }
        });
    }
    ...
    FLOW_FFI_EXPORT const char* flow_factory_get_friendly_name(FlowNodeFactoryHandle factory, const char* class_name)
    {
        FLOW_API_CALL_HANDLE({
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_UNKNOWN,
                                                             std::string("Failed to get friendly name: ") + e.what());
                return nullptr;
            }
        });
    }
                                                     ...
                                                     const char* to_type)
    {
        ...
        {
            flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_UNKNOWN,
                                                         std::string("Failed to check convertibility: ") + e.what());
            return false;
        }
    }
