...
extern "C"
{
    ...
    FLOW_FFI_EXPORT const char* flow_node_get_id(FlowNodeHandle node)
    {
        FLOW_API_CALL_HANDLE({
            ...
            if (!node_wrapper)
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_INVALID_HANDLE, "Invalid node handle");
                return nullptr;
            }
                                                       ...
                                                       FlowNodeDataHandle data)
    {
        FLOW_API_CALL({
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_PORT_NOT_FOUND,
                                                             std::string("Input port not found: ") + port_key);
                return FLOW_ERROR_PORT_NOT_FOUND;
            }
            catch (const std::exception& e)
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_UNKNOWN,
                                                             std::string("Failed to set input data: ") + e.what());
    ...
    FLOW_FFI_EXPORT FlowNodeDataHandle flow_node_get_input_data(FlowNodeHandle node, const char* port_key)
    {
        FLOW_API_CALL_HANDLE({
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_UNKNOWN,
                                                             std::string("Failed to get input data: ") + e.what());
                return nullptr;
            }
        });
    }
    ...
    FLOW_FFI_EXPORT FlowNodeDataHandle flow_node_get_output_data(FlowNodeHandle node, const char* port_key)
    {
        FLOW_API_CALL_HANDLE({
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_PORT_NOT_FOUND,
                                                             std::string("Output port not found: ") + port_key);
                return nullptr;
            }
            catch (const std::exception& e)
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_UNKNOWN,
                                                             std::string("Failed to get output data: ") + e.what());
                return nullptr;
            }
        });
    }
    ...
    FLOW_FFI_EXPORT FlowError flow_node_clear_input_data(FlowNodeHandle node, const char* port_key)
    {
        FLOW_API_CALL({
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_UNKNOWN,
                                                             std::string("Failed to clear input data: ") + e.what());
                return FLOW_ERROR_UNKNOWN;
            }
        });
    }
    ...
    FLOW_FFI_EXPORT FlowError flow_node_clear_output_data(FlowNodeHandle node, const char* port_key)
    {
        FLOW_API_CALL({
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_UNKNOWN,
                                                             std::string("Failed to clear output data: ") + e.what());
                return FLOW_ERROR_UNKNOWN;
            }
        });
    }
    ...
    FLOW_FFI_EXPORT FlowError flow_node_invoke_compute(FlowNodeHandle node)
    {
        FLOW_API_CALL({
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_COMPUTATION_FAILED,
                                                             std::string("Node computation failed: ") + e.what());
    ...
    FLOW_FFI_EXPORT bool flow_node_validate_required_inputs(FlowNodeHandle node)
    {
        ...
        {
            flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_UNKNOWN,
                                                         std::string("Failed to validate inputs: ") + e.what());
            return false;
        }
    }
    ...
    FLOW_FFI_EXPORT FlowError flow_node_start(FlowNodeHandle node)
    {
        FLOW_API_CALL({
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_UNKNOWN,
                                                             std::string("Node Start() threw: ") + e.what());
                return FLOW_ERROR_UNKNOWN;
            }
        });
    }
    ...
    FLOW_FFI_EXPORT FlowError flow_node_stop(FlowNodeHandle node)
    {
        FLOW_API_CALL({
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_UNKNOWN,
                                                             std::string("Node Stop() threw: ") + e.what());
                return FLOW_ERROR_UNKNOWN;
            }
        });
    }
    ...
    FLOW_FFI_EXPORT bool flow_node_has_connected_inputs(FlowNodeHandle node)
    {
        ...
        {
            flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_UNKNOWN,
                                                         std::string("Failed to check input connections: ") + e.what());
            return false;
        }
    }
    ...
    FLOW_FFI_EXPORT char* flow_node_save_to_json(FlowNodeHandle node)
    {
        FLOW_API_CALL_HANDLE({
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_UNKNOWN,
                                                             std::string("Failed to serialize node: ") + e.what());
                return nullptr;
            }
        });
    }
    ...
    FLOW_FFI_EXPORT FlowError flow_node_load_from_json(FlowNodeHandle node, const char* json_str)
    {
        FLOW_API_CALL({
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_UNKNOWN,
                                                             std::string("Failed to deserialize node: ") + e.what());
                return FLOW_ERROR_UNKNOWN;
            }
        });
    }
    ...
    {
        FLOW_API_CALL({
            if (!flow_ffi::validate_handle(node, "node") || !flow_ffi::validate_pointer(port_keys, "port_keys") ||
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_UNKNOWN,
                                                             std::string("Failed to get input port keys: ") + e.what());
                return FLOW_ERROR_UNKNOWN;
            }
        });
    }
    ...
    FLOW_FFI_EXPORT const char* flow_node_get_input_port_type(FlowNodeHandle node, const char* port_key)
    {
        FLOW_API_CALL_HANDLE({
            if (!flow_ffi::validate_handle(node, "node") ||
                !flow_ffi::validate_pointer(const_cast<void*>(static_cast<const void*>(port_key)), "port_key"))
            {
                return nullptr;
            }
    ...
    FLOW_FFI_EXPORT const char* flow_node_get_output_port_type(FlowNodeHandle node, const char* port_key)
    {
        FLOW_API_CALL_HANDLE({
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(
                    FLOW_ERROR_PORT_NOT_FOUND, std::string("Failed to get output port type: ") + e.what());
                return nullptr;
            }
        });
    }
                                                               ...
                                                               bool is_input_port)
    {
        FLOW_API_CALL_HANDLE({
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(
                    FLOW_ERROR_PORT_NOT_FOUND, std::string("Failed to get port description: ") + e.what());
                return nullptr;
            }
        });
    }
    ...
    std::string MapTypeToInterworkingType(std::string_view flow_type)
    {
        ...
        // a few lines below.
        if (flow_type == "int" || flow_type == "int32_t" || flow_type == "int64_t" || flow_type == "uint32_t" ||
            flow_type == "uint64_t" || flow_type == "size_t" || flow_type == TypeName_v<int> ||
            flow_type == TypeName_v<int32_t> || flow_type == TypeName_v<int64_t> || flow_type == TypeName_v<uint32_t> ||
            flow_type == TypeName_v<uint64_t> || flow_type == TypeName_v<size_t> || flow_type == TypeName_v<long> ||
            flow_type == TypeName_v<long long> || flow_type == TypeName_v<unsigned long> ||
            flow_type == TypeName_v<unsigned long long>)
        {
            return "integer";
        }
        else if (flow_type == "float" || flow_type == "double" || flow_type == TypeName_v<float> ||
        ...
        else if (flow_type == TypeName_v<std::string> || flow_type == "std::string" || flow_type == "string" ||
        ...
        FLOW_API_CALL({
            ...
            try
            {
                ...
                catch (const std::out_of_range&)
                {
                    ...
                    {
                        flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_PORT_NOT_FOUND,
                                                                     std::string("Port not found: ") + port_key);
                        return FLOW_ERROR_PORT_NOT_FOUND;
                    }
                }
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_UNKNOWN,
                                                             std::string("Failed to get port metadata: ") + e.what());
                return FLOW_ERROR_UNKNOWN;
            }
        });
    }
