...
extern "C"
{
    ...
    FLOW_FFI_EXPORT FlowModuleHandle flow_module_create(FlowNodeFactoryHandle factory)
    {
        try
        {
            ...
            return static_cast<FlowModuleHandle>(flow_ffi::create_handle<std::shared_ptr<Module>>(std::move(module)));
        }
    ...
    FLOW_FFI_EXPORT FlowError flow_module_load(FlowModuleHandle module, const char* path)
    {
        try
        {
            ...
            if (strlen(path) == 0)
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_INVALID_ARGUMENT, "Path cannot be empty");
                return FLOW_ERROR_INVALID_ARGUMENT;
            }
            ...
            if (!module_ptr)
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_INVALID_HANDLE, "Invalid module handle");
            ...
            if (!success)
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_MODULE_LOAD_FAILED, "Failed to load module");
                return FLOW_ERROR_MODULE_LOAD_FAILED;
            }
    ...
    FLOW_FFI_EXPORT FlowError flow_module_unload(FlowModuleHandle module)
    {
        try
        {
            ...
            if (!module)
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_INVALID_ARGUMENT, "Invalid module handle");
                return FLOW_ERROR_INVALID_ARGUMENT;
            }
            ...
            if (!success)
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_MODULE_LOAD_FAILED, "Failed to unload module");
                return FLOW_ERROR_MODULE_LOAD_FAILED;
            }
    ...
    FLOW_FFI_EXPORT FlowError flow_module_register_nodes(FlowModuleHandle module)
    {
        try
        {
            ...
            if (!(*module_ptr)->IsLoaded())
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_MODULE_LOAD_FAILED, "Module is not loaded");
                return FLOW_ERROR_MODULE_LOAD_FAILED;
            }
    ...
    FLOW_FFI_EXPORT const char* flow_module_get_name(FlowModuleHandle module)
    {
        try
        {
            ...
            if (!metadata)
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_MODULE_LOAD_FAILED, "No metadata available");
                return nullptr;
            }
