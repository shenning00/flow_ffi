...
SharedNodeData wrap_image(Image img) { return std::make_shared<NodeData<Image>>(std::move(img)); }
...
extern "C"
{
        ...
        FLOW_API_CALL_HANDLE({
            ...
            if (length == 0)
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_INVALID_ARGUMENT,
                                                             "Invalid argument: length must be > 0");
                return nullptr;
            }
                                                                  ...
                                                                  int32_t row_stride_bytes, FlowPixelFormat format)
    {
        FLOW_API_CALL_HANDLE({
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_INVALID_ARGUMENT,
                                                             "Invalid argument: width and height must be > 0");
                return nullptr;
            }
            ...
            const std::size_t total = static_cast<std::size_t>(row_stride_bytes) * static_cast<std::size_t>(height);
        ...
        FLOW_API_CALL({
            ...
            if (wrapper->data->Type() != TypeName_v<Image>)
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_TYPE_MISMATCH,
                                                             std::string("Expected flow::Image, got ") +
                                                                 std::string(wrapper->data->Type()));
            ...
            out_desc->kind   = (img.kind == Image::Kind::RawRGBA) ? FLOW_IMAGE_KIND_RAW_RGBA : FLOW_IMAGE_KIND_ENCODED;
