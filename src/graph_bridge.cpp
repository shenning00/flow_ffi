...
extern "C"
{
    ...
    {
        FLOW_API_CALL_HANDLE({
            if (!flow_ffi::validate_handle(graph, "graph") || !flow_ffi::validate_string(class_id, "class_id") ||
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_INVALID_HANDLE,
                                                             "Failed to get node factory from environment");
                return nullptr;
            }
            ...
            if (!node)
            {
                flow_ffi::ErrorManager::instance().set_error(
                    FLOW_ERROR_NODE_NOT_FOUND, std::string("Failed to create node of class: ") + class_id);
                return nullptr;
            }
    ...
    {
        FLOW_API_CALL({
            if (!flow_ffi::validate_handle(graph, "graph") || !flow_ffi::validate_string(node_id, "node_id"))
            {
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_INVALID_ARGUMENT,
                                                             std::string("Invalid UUID format: ") + e.what());
                return FLOW_ERROR_INVALID_ARGUMENT;
            }
    ...
    FLOW_FFI_EXPORT FlowNodeHandle flow_graph_get_node(FlowGraphHandle graph, const char* node_id)
    {
        FLOW_API_CALL_HANDLE({
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_NODE_NOT_FOUND,
                                                             std::string("Node not found with ID: ") + node_id);
                return nullptr;
            }

            flow_ffi::ErrorManager::instance().clear_error();
            return static_cast<FlowNodeHandle>(flow_ffi::get_or_create_node_handle(std::move(node)));
        });
    }
    ...
    {
        FLOW_API_CALL({
            if (!flow_ffi::validate_handle(graph, "graph") || !flow_ffi::validate_pointer(nodes, "nodes") ||
            ...
            *nodes = new FlowNodeHandle[*count];
            ...
            {
                (*nodes)[i++] = static_cast<FlowNodeHandle>(flow_ffi::get_or_create_node_handle(node));
            }
                                                                  ...
                                                                  const char* target_port)
    {
        ...
        try
        {
            if (!flow_ffi::validate_handle(graph, "graph") || !flow_ffi::validate_string(source_id, "source_id") ||
            ...
            auto connection =
                (*graph_ptr)
                    ->ConnectNodes(source_uuid, IndexableName(source_port), target_uuid, IndexableName(target_port));

            if (!connection)
            {
                flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_CONNECTION_FAILED,
                                                             "Failed to create connection between nodes");
                return nullptr;
            }
    ...
    FLOW_FFI_EXPORT FlowError flow_graph_disconnect_nodes(FlowGraphHandle graph, const char* connection_id)
    {
        FLOW_API_CALL({
            ...
            {
                flow_ffi::ErrorManager::instance().set_error(
                    FLOW_ERROR_CONNECTION_FAILED, std::string("Connection not found with ID: ") + connection_id);
                return FLOW_ERROR_CONNECTION_FAILED;
            }

            // Disconnect using the connection details
            (*graph_ptr)
                ->DisconnectNodes(connection->StartNodeID(), connection->StartPortKey(), connection->EndNodeID(),
                                  connection->EndPortKey());
    ...
    {
        FLOW_API_CALL({
            if (!flow_ffi::validate_handle(graph, "graph") || !flow_ffi::validate_string(json_str, "json_str"))
            {
                return FLOW_ERROR_INVALID_ARGUMENT;
            }
        ...
        FLOW_API_CALL({
            ...
            *connections = new FlowConnectionInfo[*count];
            ...
            for (const auto& conn_pair : graph_connections)
            {
                ...
                std::strcpy(const_cast<char*>((*connections)[index].source_node_id), start_id_str.c_str());

                (*connections)[index].source_port_key = new char[start_port_str.length() + 1];
                std::strcpy(const_cast<char*>((*connections)[index].source_port_key), start_port_str.c_str());

                (*connections)[index].target_node_id = new char[end_id_str.length() + 1];
                std::strcpy(const_cast<char*>((*connections)[index].target_node_id), end_id_str.c_str());

                (*connections)[index].target_port_key = new char[end_port_str.length() + 1];
                std::strcpy(const_cast<char*>((*connections)[index].target_port_key), end_port_str.c_str());
                                                ...
                                                const char* target_id, const char* target_port)
    {
        ...
        if (!graph_ptr || !*graph_ptr)
        {
            flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_INVALID_HANDLE, "Failed to get graph from handle");
            return false;
        }
        ...
        {
            flow_ffi::ErrorManager::instance().set_error(FLOW_ERROR_INVALID_ARGUMENT,
                                                         std::string("Failed to validate connection: ") + e.what());
            return false;
        }
    }
