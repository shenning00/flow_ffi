...
static int mock_create(void* /*reg*/, int32_t w, int32_t h,
                        void** out_obj, int64_t* out_id)
{
    std::lock_guard<std::mutex> lk(g_mock_mutex);
    ++g_mock.create_call_count;
    if (g_mock.create_should_fail) return -1;
    ++s_texture_obj_tag;
    g_mock.texture_object = reinterpret_cast<void*>(
        static_cast<uintptr_t>(s_texture_obj_tag));
...
static int mock_submit_frame(void* /*obj*/,
                              const void* src,
                              int32_t     stride,
                              int32_t     w,
                              int32_t     h)
{
    ...
    const size_t bytes = static_cast<size_t>(stride) *
                         static_cast<size_t>(h);
    g_mock.submitted_bytes.assign(
...
TEST(FlowFlutterTextureSink, BytesRoundtrip) {
    ...
    ASSERT_EQ(flow_ffi_upload_to_texture(handle, pixels.data(), w * 4, w, h),
              FLOW_SUCCESS);

    {
        ...
        EXPECT_EQ(std::memcmp(g_mock.submitted_bytes.data(),
...
TEST(FlowFlutterTextureSink, NodeClassRegistered) {
    ...
    EXPECT_TRUE(found)
        << "\"" << expected_class << "\" should be registered in the "
...
TEST(FlowFlutterTextureSink, ComputeNoInput_NoOp) {
    ...
    node.OnError.Bind(flow::IndexableName{"err"},
                      [&](const std::exception& ex) { error_fired = true; (void)ex; });
    node.InvokeCompute();
