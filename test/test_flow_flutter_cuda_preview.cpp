...
TEST_F(FlowFlutterCudaPreviewTest, ComputeWithNoInputIsNoOp) {
    ...
    node.OnError.Bind(flow::IndexableName{"test_listener"},
                      [&](const std::exception& /*e*/) { error_fired = true; });
...
TEST_F(FlowFlutterCudaPreviewTest, ComputeWithMockOpsAndCudaBuffer) {
    ...
    node.Stop();
    EXPECT_EQ(g_mock.destroy_call_count, 2)
        << "Stop() must unregister the texture (destroy called)";
}
