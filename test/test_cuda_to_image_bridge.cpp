...
int main() {
    ...
    if (!out_data) {
        std::fprintf(stderr,
                     "FAIL: CudaToImage produced no output on 'out' port\n");
        return 1;
    }
    ...
    for (std::size_t i = 0; i < TOTAL; ++i) {
        if (bytes[i] != FILL_BYTE) {
            std::fprintf(stderr,
                         "FAIL: bytes[%zu] = 0x%02X, expected 0x%02X\n",
                         i, bytes[i], FILL_BYTE);
            return 1;
        }
    }
